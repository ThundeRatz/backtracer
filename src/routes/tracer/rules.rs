...
use crate::{db, models::rule::Rule, schema::tracer::rules};
...
#[openapi(tag = "Rules")]
#[get("/rules/<id>")]
pub async fn get_rule_id(id: i32, conn: db::DbConn) -> Option<Json<Rule>> {
    let rule: Rule = conn
...
#[openapi(skip)]
#[get("/rules/<id>/go")]
pub async fn redirect_to_rule(id: i32, conn: db::DbConn) -> Option<Redirect> {
    let rule: Rule = conn
        .run(move |c| rules::table.find(id).first(c))
        .await
        .ok()?;
